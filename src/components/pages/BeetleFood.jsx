...
import LocalBeetleImg1 from "../../assets/Beetle/575096516_1268426861638529_5390863248606645428_n.jpg";
...
import LocalBeetleImg6 from "../../assets/Beetle/554156924_1749940556411142_1315520537122181520_n.jpg";
import LocalBeetleImg7 from "../../assets/Beetle/568671993_816941147622342_3814503822687149354_n.jpg";
import LocalBeetleImg8 from "../../assets/Beetle/573277632_1497056174841558_1541750357781069948_n.jpg";
// ================== ลิงก์ติดต่อ ==================
const LINKS = {
...
// ================== สินค้า (เพิ่ม stock ได้ ถ้าไม่ใส่ถือว่าไม่จำกัด) ==================
const products = [
  {
    id: 24,
  ...
  },
  {
    ...
    description: "มูชิคิงไต้หวัน:ตัวอ่อน:L3: 1 คู่",
    tags: ["WF1"],
    stock: 1, // ❗ สินค้าหมด
  },
...

  {
    ...
    description:
      "ด้วงกว่างเฮอร์คิวลิส:ต้นตระกูลสายหนาจากญี่ปุ่น: ตัวอ่อน:L3: 1 คู่",
    tags: ["CBF1"],
    stock: 1, // ❗ สินค้าหมด
  },

 {
    ...
    description: "กว่างชนตาขาว:ตัวอ่อน:L1-2: 1 ตัว",
    tags: ["CBF1"],
    stock: 5, // ❗ สินค้าหมด
   ...
   {
    id: 29,
    name: "Dicronorhina derbyana",
    price: 300,
    image: LocalBeetleImg6,
    description:
      "ด้วงดอกไม้เดอเบียน่า: 1 ตัว",
    tags: ["CBF2"],
    stock: 1, // ❗ สินค้าหมด
  },
 ...
 {
    id: 30,
    name: "chalcosoma moellenkampi ",
    price: 1000,
    image: LocalBeetleImg7,
    description: "ด้วง 3 เขา โมเลนแคมป์:ตัวอ่อน:L3: 1 คู่",
    tags: ["CBF1"],
    stock: 0, // ❗ สินค้าหมด
  },
...

  {
    ...
    description: "เนปจูน:ตัวอ่อน:L1-2: 1 ตัว",
    tags: ["CBF1", "122xff"],
    stock: 0, // ❗ สินค้าหมด
  },
];
...
/* ================== Shipping ================== */
// ❗ ไม่มีตัวเลขค่าส่งบนการ์ด + ไม่มีไอคอน
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
            <div>
              <label className="text-xs text-gray-600 dark:text-gray-300">ชื่อผู้รับ</label>
              <input
                value={buyerName}
            ...
            <div>
              <label className="text-xs text-gray-600 dark:text-gray-300">เบอร์โทร</label>
              <input
                value={phone}
            ...
            <div className="sm:col-span-2">
              <label className="text-xs text-gray-600 dark:text-gray-300">ที่อยู่จัดส่ง</label>
              <textarea
                value={address}
          ...
          <div>
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-extrabold">ShopBeetle — ใบสรุปคำสั่งซื้อ</h3>
              ...
              <div className="text-sm">
                <p><span className="font-semibold">ผู้รับ:</span> {buyerName || "-"}</p>
                <p><span className="font-semibold">โทร:</span> {phone || "-"}</p>
                <p><span className="font-semibold">ที่อยู่:</span> {address || "-"}</p>
              </div>
              ...
              <div className="mt-4 border rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {cart.items.length === 0 ? (
                      <tr>
                        <td colSpan={4} className="p-4 text-center text-gray-500">ไม่มีสินค้า</td>
                      </tr>
                    ) : (
                      cart.items.map((it) => (
                        <tr key={it.id} className="border-t">
                          <td className="p-2">{it.name}</td>
                          <td className="p-2 text-right">฿{formatTHB(it.price)}</td>
                          <td className="p-2 text-center">{it.qty}</td>
                          <td className="p-2 text-right">฿{formatTHB(it.price * it.qty)}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
              ...
              <div className="mt-4 space-y-1 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">฿{formatTHB(cart.subtotal)}</span>
                </div>
                <div className="flex justify-between">
                  <span>ค่าส่ง</span>
                  <span className="font-semibold">฿{formatTHB(shippingCost)}</span>
                </div>
            ...
            <div className="flex items-center justify-between gap-2">
                  ...
                  {Object.values(SHIPPING_METHODS).map((opt) => (
                    <button
                      ...
                      className={`px-3 py-2 rounded-md text-xs sm:text-sm font-semibold text-white transition
                        ${(SHIPPING_METHODS[shipping] ?? SHIPPING_METHODS.flash).id === opt.id
              ...
              <div className="text-right">
                <div className="text-xs text-gray-600 dark:text-gray-300">
                  สินค้า ฿{formatTHB(cart.subtotal)} + ค่าส่ง ฿{formatTHB(shippingCost)}
                </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className={`px-3 py-3 rounded-lg text-sm sm:text-base font-semibold text-white
                  ${cart.items.length === 0 ? "bg-gray-400 opacity-60" : "bg-gray-900 dark:bg-gray-800"}`}
                disabled={cart.items.length === 0}
              ...
              <a
                ...
                className={`col-span-2 inline-flex items-center justify-center px-4 py-3 rounded-lg text-sm sm:text-base font-bold text-white shadow
                  ${cart.items.length === 0 ? "pointer-events-none opacity-50 bg-gray-400" : `bg-gradient-to-r ${ship.from} ${ship.to}`}`}
              >
...
/* ================== การ์ดสินค้า — ไม่แสดงตัวเลขค่าส่งบนการ์ด + สต็อกหมด ================== */
...
export default function Beetle() {
  ...
  // เพิ่มสินค้าแล้วเปิดตะกร้าอัตโนมัติ (กันเพิ่มถ้าสต็อกหมด)
  const handleAdd = (p) => {
